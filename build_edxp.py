...
def zip_main_edxp(args):
    ...
    name = 'zip_system_edxp_v0462_install.zip'
    ...
    with zipfile.ZipFile(output, 'w', compression=zipfile.ZIP_DEFLATED, allowZip64=False) as zipf:
        ...
        # updater-script
        source = op.join('scripts', 'flash_script_edxp.sh')
        target = op.join('META-INF', 'com', 'google',
        ...
        # edxp - jars
        source = op.join('edxp', 'edconfig.jar')
        target = op.join('edxp', 'edconfig.jar')
        zip_with_msg(zipf, source, target)
        source = op.join('edxp', 'eddalvikdx.jar')
        target = op.join('edxp', 'eddalvikdx.jar')
        zip_with_msg(zipf, source, target)
        source = op.join('edxp', 'eddexmaker.jar')
        target = op.join('edxp', 'eddexmaker.jar')
        zip_with_msg(zipf, source, target)
        source = op.join('edxp', 'edxp.jar')
        target = op.join('edxp', 'edxp.jar')
        zip_with_msg(zipf, source, target)

        # edxp - binaries
        source = op.join('edxp', 'espf.arm')
        target = op.join('edxp', 'espf.arm')
        zip_with_msg(zipf, source, target)
        source = op.join('edxp', 'espf.arm64')
        target = op.join('edxp', 'espf.arm64')
        zip_with_msg(zipf, source, target)
        source = op.join('edxp', 'zygote_restart.arm')
        target = op.join('edxp', 'zygote_restart.arm')
        zip_with_msg(zipf, source, target)
        source = op.join('edxp', 'zygote_restart.arm64')
        target = op.join('edxp', 'zygote_restart.arm64')
        zip_with_msg(zipf, source, target)

        # edxp - libraries
        source = op.join('edxp', 'libmemtrack.so.arm')
        target = op.join('edxp', 'libmemtrack.so.arm')
        zip_with_msg(zipf, source, target)
        source = op.join('edxp', 'libmemtrack.so.arm64')
        target = op.join('edxp', 'libmemtrack.so.arm64')
        zip_with_msg(zipf, source, target)
        source = op.join('edxp', 'libriru_edxp.so.arm')
        target = op.join('edxp', 'libriru_edxp.so.arm')
        zip_with_msg(zipf, source, target)
        source = op.join('edxp', 'libriru_edxp.so.arm64')
        target = op.join('edxp', 'libriru_edxp.so.arm64')
        zip_with_msg(zipf, source, target)
        source = op.join('edxp', 'libwhale.edxp.so.arm')
        target = op.join('edxp', 'libwhale.edxp.so.arm')
        zip_with_msg(zipf, source, target)
        source = op.join('edxp', 'libwhale.edxp.so.arm64')
        target = op.join('edxp', 'libwhale.edxp.so.arm64')
        zip_with_msg(zipf, source, target)

        # edxp - init.d script
        source = op.join('edxp', 'initdscript')
        target = op.join('edxp', 'initdscript')
        zip_with_msg(zipf, source, target)
...
def zip_uninstaller_edxp(args):
    ...
    with zipfile.ZipFile(output, 'w', compression=zipfile.ZIP_DEFLATED, allowZip64=False) as zipf:
        ...
        zipf.writestr(target, gen_update_binary())
        # updater-script
        source = op.join('scripts', 'magisk_uninstaller_edxp.sh')
        target = op.join('META-INF', 'com', 'google',
                         'android', 'updater-script')
        zip_with_msg(zipf, source, target)
